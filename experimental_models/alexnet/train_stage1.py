"""
Stage 1: Full Training - AlexNet
All layers trainable from the start (no pretrained weights available).
Hyperparameter Optimisation: Unlike pretrained models, AlexNet requires
...
def create_model():
    """Create AlexNet model with ALL layers trainable.
    AlexNet architecture built manually (not available in tf.keras.applications).
    ...
    learned features, AlexNet has no pretrained weights. Freezing random
    weights would prevent convergence. Therefore, all layers are trainable.
    """
    # Build AlexNet convolutional base
...
def main():
    print("\n" + "="*60)
    print("STAGE 1: FULL TRAINING - AlexNet")
    ...
    print("Creating AlexNet model (all layers trainable)...")
    model = create_model()
    ...
    print("\n" + "="*60)
    print("Starting Stage 1 Training - AlexNet (All Layers Unfrozen)")
    print("="*60 + "\n")
    ...
    print("\n" + "="*60)
    print(f"STAGE 1 COMPLETE! - AlexNet")

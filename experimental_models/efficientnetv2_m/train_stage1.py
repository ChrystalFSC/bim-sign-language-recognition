"""
Stage 1: Feature Extraction Training - EfficientNetV2-M
Only train the top classification layers while keeping EfficientNetV2-M base frozen.
"""
...
def create_model():
    base = keras.applications.EfficientNetV2M(input_shape=CONFIG["input_shape"], include_top=False, weights='imagenet', include_preprocessing=False)
...
def main():
    print("\n" + "="*60 + "\nSTAGE 1: FEATURE EXTRACTION - EfficientNetV2-M\nBase model FROZEN\n" + "="*60)
    ...
    print(f"\nSTAGE 1 COMPLETE! - EfficientNetV2-M | Best Val Acc: {max(history.history['val_accuracy'])*100:.2f}%")

"""
Stage 1: Feature Extraction Training - MobileNetV2
Only train the top classification layers while keeping MobileNetV2 base frozen.
"""
...
def create_model():
    base = keras.applications.MobileNetV2(input_shape=CONFIG["input_shape"], include_top=False, weights='imagenet')
...
def main():
    print("\n" + "="*60 + "\nSTAGE 1: FEATURE EXTRACTION - MobileNetV2\nBase model FROZEN\n" + "="*60)
    ...
    print(f"\nSTAGE 1 COMPLETE! - MobileNetV2 | Best Val Acc: {max(history.history['val_accuracy'])*100:.2f}%")

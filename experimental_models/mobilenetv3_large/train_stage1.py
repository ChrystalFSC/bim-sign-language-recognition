"""
Stage 1: Feature Extraction Training - MobileNetV3-Large
Only train the top classification layers while keeping MobileNetV3-Large base frozen.
"""
...
def create_model():
    base = keras.applications.MobileNetV3Large(input_shape=CONFIG["input_shape"], include_top=False, weights='imagenet', include_preprocessing=False, minimalistic=False, alpha=1.0)
...
def main():
    print("\n" + "="*60 + "\nSTAGE 1: FEATURE EXTRACTION - MobileNetV3-Large\nBase model FROZEN\n" + "="*60)
    ...
    print(f"\nSTAGE 1 COMPLETE! - MobileNetV3-Large | Best Val Acc: {max(history.history['val_accuracy'])*100:.2f}%")

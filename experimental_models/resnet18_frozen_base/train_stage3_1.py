"""
Stage 3.1: Terminal Learning Rate Squeeze - ResNet18 (Frozen Base)
"""
...
CONFIG = {"input_shape": (224, 224, 3), "batch_size": 64, "epochs": 20, "learning_rate": 1e-6,
    "focal_gamma": 5.0, "focal_alpha": 0.25, "model_path": "output/best_model_stage3.keras",
...
def main():
    print("\n" + "="*60 + "\nSTAGE 3.1: THE LAST SQUEEZE - ResNet18 (Frozen Base)\nAggressive Focal Loss (gamma=5.0)\n" + "="*60 + "\n")
    np.random.seed(CONFIG["seed"]); tf.random.set_seed(CONFIG["seed"])
    tf.keras.mixed_precision.set_global_policy('mixed_float16')
    model = keras.models.load_model(CONFIG["model_path"], custom_objects={'focal_loss_fn': focal_loss(2.0, 0.25)})
    ...
    model = keras.models.load_model(os.path.join(CONFIG["output_dir"], 'best_model_stage3_1.keras'), custom_objects={'focal_loss_fn': focal_loss(CONFIG["focal_gamma"], CONFIG["focal_alpha"])})

"""
Stage 3.1: Terminal Learning Rate Squeeze - VGG19
...
def main():
    print("\n" + "="*60 + "\nSTAGE 3.1: THE LAST SQUEEZE - VGG19\nAggressive Focal Loss (gamma=5.0)\n" + "="*60 + "\n")
    ...
    with open(os.path.join(CONFIG["output_dir"], 'per_class_performance_stage3_1.txt'), 'w') as f:
        f.write(f"STAGE 3.1 - DETAILED PER-CLASS PERFORMANCE - VGG19\n")
        f.write("="*70 + "\n\nOVERALL METRICS\n" + "-"*70 + "\n")
    ...
    plt.title('Confusion Matrix - Stage 3.1 (Aggressive Focal Loss) - VGG19')
    plt.xlabel('Predicted'); plt.ylabel('True'); plt.tight_layout()
    plt.savefig(os.path.join(CONFIG["output_dir"], 'confusion_matrix_stage3_1.png'), dpi=150); plt.close()
    print(f"\nSTAGE 3.1 COMPLETE! - VGG19 | Model saved: output/best_model_stage3_1.keras")
